...
namespace SaiCommon
{
	...
	public:
		...
		/**
		 * @brief Enables tag-based monitoring to profile specific blocks of code.
		 * @param enable Whether tag monitoring is active.
		 * @param print_interval_seconds How often (in seconds) the summary should be printed.
		 */
		void enableTagMonitoring(bool enable, double print_interval_seconds = 1.0);
		/**
		 * @brief Start timing a specific block of code.
		 * * @param tag_name The name of the block (e.g., "controls", "graphics").
		 */
		void openTag(const std::string &tag_name);

		/**
		 * @brief Stop timing a specific block of code.
		 * * @param tag_name The name of the block. Must match the name used in openTag.
		 */
		void closeTag(const std::string &tag_name);

		/**
		 * @brief Prints a formatted summary of the tagged code blocks.
		 * @details This should be called once per loop iteration. It will only print
		 * if the specified print_interval_ms has elapsed to avoid slowing down the loop.
		 */
		void printMonitoringSummary();

		/**
		 * @brief RAII helper for scoped tag monitoring.
		 * @details Automatically opens the tag on creation and closes it when it goes out of scope.
		 */
		struct ScopedBlock
		{
			LoopTimer &timer;
			std::string name;
			ScopedBlock(LoopTimer &t, const std::string &n) : timer(t), name(n) { timer.openTag(name); }
			~ScopedBlock() { timer.closeTag(name); }
		};
	...
	private:
		...
		bool tag_monitoring_enabled_ = false;
		double tag_monitor_print_interval_ms_ = 1000.0;

		std::chrono::high_resolution_clock::time_point t_last_tag_print_;
		unsigned long long tags_window_loop_count_ = 0;

		std::vector<std::string> tag_order_; // Preserves the order tags are created for printing
		std::unordered_map<std::string, TagData> tags_;

		double tag_monitor_print_interval_s_ = 1.0;
		double accumulated_wait_time_ms_ = 0.0;
	};
