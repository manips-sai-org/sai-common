...
namespace SaiCommon
{
	...
	void RedisClient::ping()
	{
		...
		if (!reply)
			throw std::runtime_error("RedisClient: PING failed.");
	...
	void RedisClient::setBinary(const std::string &key, const std::vector<unsigned char> &value)
	{
		// Call SET command
		...
		for (const auto &group_name : group_names)
		{
			for (int i = 0; i < _objects_to_receive.at(group_name).size(); ++i)
			{
				...
				case EIGEN_OBJECT:
				{
					...
					// Decode the string into an Eigen matrix
					Eigen::MatrixXd tmp_return_matrix =
						RedisClient::decodeEigenMatrix(return_values[return_values_index]);

					int nrows = tmp_return_matrix.rows();
					int ncols = tmp_return_matrix.cols();

					// Assuming your double* buffer expects standard Column-Major layout.
					// Map the raw pointer and let Eigen handle the highly-optimized memory copy.
					Eigen::Map<Eigen::MatrixXd> dest_map(tmp_pointer, nrows, ncols);
					dest_map = tmp_return_matrix;
				}
				break;
	...
	static inline Eigen::MatrixXd decodeEigenMatrixWithDelimiters(
		...
		const std::string &delimiter_set, size_t idx_row_end = std::string::npos)
	{
		// 1. O(1) Lookup Table for lightning-fast character skipping
		// A 256-byte array fits perfectly in the CPU L1 cache.
		bool is_ignored[256] = {false};
		is_ignored[static_cast<unsigned char>(' ')] = true;
		is_ignored[static_cast<unsigned char>('\t')] = true;
		is_ignored[static_cast<unsigned char>('\n')] = true;
		is_ignored[static_cast<unsigned char>('\r')] = true;
		is_ignored[static_cast<unsigned char>(col_delimiter)] = true;

		for (char c : delimiter_set)
		{
			is_ignored[static_cast<unsigned char>(c)] = true;
		}
		// Ensure the row delimiter is NEVER ignored so we can trigger row transitions
		is_ignored[static_cast<unsigned char>(row_delimiter)] = false;

		// 2. Pre-allocated contiguous buffer to minimize heap allocations
		std::vector<double> values;
		values.reserve(64); // Adjust this default if your matrices are usually larger

		size_t current_row_cols = 0;
		size_t num_cols = 0;
		size_t num_rows = 1;

		// 3. Raw pointer bounds for zero-overhead traversal
		const char *ptr = str.data();
		const char *end = ptr + std::min(str.size(), idx_row_end);

		// 4. Single-pass ultra-fast parse
		while (ptr < end)
		{
			// Fast-forward past ignored characters (spaces, commas, brackets, etc.)
			while (ptr < end && is_ignored[static_cast<unsigned char>(*ptr)])
			{
				ptr++;
			}
			...
			// Check for row transition
			if (*ptr == row_delimiter)
			{
				if (num_cols == 0)
			...
			// Parse the float directly from the memory buffer
			char *endptr;
			...
			if (endptr > ptr)
			{
				...
				current_row_cols++;
				ptr = endptr; // Jump pointer to the exact end of the parsed number
			}
			else
			{
				// Failsafe: step over unexpected non-delimiter/non-numeric junk
				ptr++;
			}
		}

		// 5. Matrix dimension resolution
		if (current_row_cols > 0 && num_cols == 0)
		{
			num_cols = current_row_cols;
		}
		else if (current_row_cols == 0 && num_rows > 1)
		{
			num_rows--; // Adjust if there was a trailing row delimiter
		}

		if (num_rows == 1 && !values.empty())
		{
			num_rows = num_cols;
			num_cols = 1;
		}
		...
		// Validate structure
		if (values.empty() || values.size() != num_rows * num_cols)
		{
			throw std::runtime_error("RedisClient: Failed to decode Eigen Matrix from: " + str);
		}

		// 6. Zero-loop Eigen mapping
		// Reads directly from our vector memory and constructs the MatrixXd in one shot
		return Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(
			values.data(), num_rows, num_cols);
	}
	...
	Eigen::MatrixXd RedisClient::decodeEigenMatrix(const std::string &str)
	{
		...
		if (idx_row_end != std::string::npos)
		{
			...
			if (idx_temp != std::string::npos)
				idx_row_end = idx_temp;
		}
